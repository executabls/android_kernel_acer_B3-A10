    ...
    for (; tags->hdr.size; tags = tag_next(tags)) {
	    ...
	    bl_mem_sz += tags->u.mem.size;
            ...
            cmdline_tag = tags;
                ...
                printk("u.boot.bootmode=%d\n", tags->u.boot.bootmode);
                //lupingzhong_add factory mode start
                if(tags->u.boot.bootmode == HQ_FACTORY_BOOT) //APK Test mode startup
                {
                        g_boot_mode = NORMAL_BOOT;
                        g_boot_mode_ex = HQ_FACTORY_BOOT;
                }
                else if(tags->u.boot.bootmode == 12)// acer demo mode haojun
			{
				g_boot_mode = NORMAL_BOOT;
				g_boot_mode_ex = 12;
			}
			else if(tags->u.boot.bootmode == 13)// acer lk wipe data
			{
