...
static ssize_t gt9xx_debug_read1(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
        unsigned char mode[1];
        int ret = 1;
        int boot_mode_ex = get_boot_mode_ex();
        printk("lpz boot_mode_ex=%d\n",boot_mode_ex);
        if (HQ_FACTORY_BOOT== boot_mode_ex){
                printk("lpz boot_mode_ex_11 proc_read success\n");
                mode[0] = 1;
        } 
        else if(12 == boot_mode_ex)
        {
                printk("lpz boot_mode_ex_12 proc_read success\n");
                mode[0] = 2;
        }
        else if(13 == boot_mode_ex)// //demomode
        {
                printk("lpz boot_mode_ex_13 proc_read success\n");
                mode[0] = 3;
        }
        else
        {
...
static const struct file_operations gt9xx_proc_fops = {
        .read = gt9xx_debug_read1,
};

int fts_create_proc_boot_status0(struct i2c_client * client)
{
        fts_proc_entry1 = proc_create(PROC_NAME1, 0777, NULL, &gt9xx_proc_fops);
...
reset_proc:
 ...
 ///////////////////////////////////////////////
	...
	err_fw  = i2c_smbus_read_i2c_block_data(i2c_client, 0xA6, 1, &fw_version);
	...
	tpd_load_status = 1;
//fts_create_proc_boot_status0(client); //lupingzhong add for factory APK
...
static int tpd_local_init(void)
{
    ...
    hardware_info.tpd_fw_version = fw_version;  // lgx
    tpd_type_cap = 1;
    return 0; 
 }
