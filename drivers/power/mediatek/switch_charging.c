...
void pchr_turn_on_charging (void)
{
	...
	if (BMT_status.bat_charging_state == CHR_ERROR) {
		...
		if((1 == g_bcct_flag) && (g_temp_CC_value < CHARGE_CURRENT_500_00_MA)) { 
			bq24296_set_force_20pct(0x1); 
			g_temp_CC_value = g_temp_CC_value * 5; 
			battery_log(BAT_LOG_CRTI, "Enable 20 PCT\n"); 
		} else { 
			bq24296_set_force_20pct(0x0); 
		} 
    ...
    {
        charging_enable = KAL_FALSE;
		//add by zym,modify disable power path,in RunIn Test
