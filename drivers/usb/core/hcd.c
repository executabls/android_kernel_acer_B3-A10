...
#if USB_URB_TASKLET_CB
static void usb_urb_tasklet(unsigned long data)
{
...
static int unlink1(struct usb_hcd *hcd, struct urb *urb, int status)
{
	...
	if (is_root_hub(urb->dev))
		...
		#if USB_URB_TASKLET_CB
		tasklet_kill(&urb->tasklet); //may cause tasklet no callback
		#endif
	}
	return value;
}
...
void usb_hcd_giveback_urb(struct usb_hcd *hcd, struct urb *urb, int status)
{
	...
	urb->status = status;
	#if USB_URB_TASKLET_CB
	tasklet_init(&urb->tasklet, usb_urb_tasklet, (unsigned long)urb);
	tasklet_hi_schedule(&urb->tasklet);
	#else
	urb->complete (urb);
	atomic_dec (&urb->use_count);
	if (unlikely(atomic_read(&urb->reject)))
