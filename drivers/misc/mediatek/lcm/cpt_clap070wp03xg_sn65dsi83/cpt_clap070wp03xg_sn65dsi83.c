...
{	
	...
	//params->dsi.mode   =SYNC_EVENT_VDO_MODE ;
       ...
       params->dsi.word_count=800*3;
		...
		params->dsi.vertical_sync_active= 4; //10;//10;
		params->dsi.vertical_backporch= 8 ;//==========10;//12;
		params->dsi.vertical_frontporch= 4 ;//=========15;//9;
		params->dsi.vertical_active_line= FRAME_HEIGHT;//hight

		params->dsi.horizontal_sync_active				= 32 ; //110;//100;  //
		params->dsi.horizontal_backporch				= 21 ;//===========110;//80; //
		params->dsi.horizontal_frontporch				=  147 ;//===============100;//100; //
		params->dsi.horizontal_active_pixel				= FRAME_WIDTH;//=wight

		params->dsi.ssc_disable = 1;//huangyisong_add to fix RF issue  20141205
		params->dsi.PS=LCM_PACKED_PS_24BIT_RGB888;
		params->dsi.pll_select=0;	//0: MIPI_PLL; 1: LVDS_PLL
		params->dsi.PLL_CLOCK = 218;//156;//this value must be in MTK suggested table
        params->dsi.noncont_clock = 1;//if not config this para, must config other 7 or 3 paras to gen. PLL
}
#if 0
static void lcm_get_params(LCM_PARAMS *params)
{	
...
static void lcm_init(void)
	...
	printf("tM070ddh06--BUILD_LK-6-lcm_init \n");	
...
#else
	printk("tM070ddh06--kernel-4-lcm_init \n");	
		   mt_set_gpio_mode(LVDS_LCM_VCC, GPIO_MODE_00);	
		   mt_set_gpio_dir(LVDS_LCM_VCC, GPIO_DIR_OUT);
		   mt_set_gpio_out(LVDS_LCM_VCC, GPIO_OUT_ONE); // / LCM VCC :enable LCD VCC
